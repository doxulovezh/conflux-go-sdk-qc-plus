...
func (m *AccountManager) Unlock(address types.Address, passphrase string, codestr string) error {
	// account, err := m.account(address)
	var account accounts.Account
	addr := address.MustGetCommonAddress()
	fmt.Println("accounts.Account", addr)
	account.Address = addr
	return m.ks.Unlock(account, passphrase, codestr)
}

// // UnlockDefault unlocks the default account indefinitely.
func (m *AccountManager) UnlockDefault(passphrase string, codestr string) error {
	defaultAccount, err := m.GetDefault()
	if err != nil {
		return err
	}
	return m.Unlock(*defaultAccount, passphrase, codestr)
}

// TimedUnlock unlocks the specified account for a period of time.
func (m *AccountManager) TimedUnlock(address types.Address, passphrase string, timeout time.Duration, codestr string) error {
	account, err := m.account(address)
	if err != nil {
		return err
	}
	return m.ks.TimedUnlock(account, passphrase, timeout, codestr)
}

// TimedUnlockDefault unlocks the specified account for a period of time.
func (m *AccountManager) TimedUnlockDefault(passphrase string, timeout time.Duration, codestr string) error {
	defaultAccount, err := m.GetDefault()
	if err != nil {
		return err
	}
	return m.TimedUnlock(*defaultAccount, passphrase, timeout, codestr)
}
...
func (m *AccountManager) SignTransaction(tx types.UnsignedTransaction) ([]byte, error) {
	...
	var account accounts.Account
	cmmadr, _, err := (*tx.From).ToCommon() //不对  hash不对
	account.Address = cmmadr
	if err != nil {
		return nil, err
	}
